...
// Div que contiene la tabla de los cursos añadidos al carrito
const carrito = document.querySelector('#carrito');
// Aquí es donde se van a colocar los elementos - la tabla del menu del carrito (esta dentro de #carrito).
...
// * 117. Mostrando en el Carrito el curso seleccionado-------------------------------
...
function agregarCurso(e){
    ...
    // Para mostrar las clases del objeto cliqueado
    // for (const c in e.target.classList) {
    //     if (Object.hasOwnProperty.call(e.target.classList, c)) {
...
// # - Modifica la información del la lista de los articulos del carrito.
// # - Invoca al metodo carritoHTML() que es el que limpia la lista del carrito de HTML y
// #   toma la nueva lista modificada y la agrega al HTML.
function leerDatosCurso(curso){
    ...
    // Si es que es verdad que existe, entonces...
    if (existe) {
        ...
        const cursos = articulosCarrito.map(curso => {
            ...
            if(curso.id === infoCurso.id){
                ...
                // Si no retornamos el objeto como está para que este dentro del nuevo array (cursos)
                return curso;
            }
        });
    ...
    // Se utilizo en la clase 119, pero se reubico arriba.
    // articulosCarrito = [...articulosCarrito, infoCurso]; 

    // # Agregando elementos al arreglo de carrito

    console.log(articulosCarrito); // Para mirar los elementos del carrito.
    
    // ? Este método agregará la información a la lista del carrito en el menu superior
    carritoHTML();
...
function carritoHTML(){
    ...
    articulosCarrito.forEach( curso => {
        ...
        row.innerHTML = `
            ...
            <td>${precio}</td>
            <td>${cantidad}</td>
            <td><a href="#" id="borrar-curso" class="borrar-curso" data-id="${id}"> X </a></td>
        `;
...
function limpiarHTML(){
    ...
    while(contenedorCarrito.firstChild){
        ...
        // ? Al eliminar todos los hijos se dejará de ejecutar el while.
